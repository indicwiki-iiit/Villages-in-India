...
def render_paths(sample_df, idx):
    ...
    record['if_no_dist_nat_highway'] = sample_df.loc[idx, '(If National Highway not available within the village, '+ 
    ...
    record['if_no_dist_state_highway'] = sample_df.loc[idx, '(If State Highway not available within the village, '+ 
    'the distance range code of nearest place where facility is available is given viz; a for < 5 Kms, b for 5-10 Kms and ' +
    'c for 10+ kms ). ']

    record['major_dist_road_s'] = type_check(sample_df.loc[idx, 'Major District Road (Status A(1)/NA(2))'])
    record['if_no_dist_major_road'] = sample_df.loc[idx, '(If Major District Road not available within the village, '+ 
    'the distance range code of nearest place where facility is available is given viz; a for < 5 Kms, b for 5-10 Kms and ' +
    'c for 10+ kms ). ']
