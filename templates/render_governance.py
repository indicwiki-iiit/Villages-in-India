...
def render_governance(sample_df, idx):
    ...
    record['subdist_HQ_dist'] = NACheck(sample_df.loc[idx, 'Sub District Head Quarter (Distance in km)'])
    record['district_HQ'] = NACheck(sample_df.loc[idx, 'District Head Quarter (Name) Telugu'])
    record['district_HQ_dist'] = NACheck(sample_df.loc[idx, 'District Head Quarter (Distance in km)'])
    record['statutory_town'] = NACheck(sample_df.loc[idx, 'Nearest Statutory Town (Name) Telugu'])
    record['statutory_town_dist'] = NACheck(sample_df.loc[idx, 'Nearest Statutory Town (Distance in km)'])
    ...
    record['if_no_dist_PS'] = NACheck(sample_df.loc[idx, '(If Assembly Polling Station not available within the village, '+
