...
translated_education_array = ['పూర్వ ప్రాథమిక పాఠశాలలు',
                              ...
                              'నాన్ ఫార్మల్ శిక్షణా కేంద్రాలు',
...
def get_landline_status(info_df, k):
    if type_check(info_df.loc[k, ('Telephone (landlines) (Status A(1)/NA(2))')]) > 1:
        ...
        if pd.isna(distance_var):
            return 'గ్రామంలో ల్యాండ్లైన్ టెలిఫోన్ సౌకార్యం లేదు'
        
        elif distance_var == 'c':
            return 'ల్యాండ్లైన్ టెలిఫోన్ సౌకార్యం గ్రామానికి 10 కి.మీ. కంటే ఎక్కువ దూరంలో లో ఉంది'
        elif distance_var == 'b':
            return 'ల్యాండ్లైన్ టెలిఫోన్ సౌకార్యం గ్రామానికి 5-10 కి.మీ. సమీపం లో ఉంది'
        return 'ల్యాండ్లైన్ టెలిఫోన్ సౌకార్యం గ్రామానికి 5 కి.మీ. కంటే తక్కువ సమీపంలో ఉన్నవి'
    
    return 'గ్రామంలో ల్యాండ్లైన్ టెలిఫోన్ సౌకార్యం ఉంది'


# In[86]:


def get_public_call_status(info_df, k):
...
def get_mobile_coverage_status(info_df, k):
    if type_check(info_df.loc[k, ('Mobile Phone Coverage (Status A(1)/NA(2))')]) > 1:
        ...
        if pd.isna(distance_var):
            return 'మొబైల్ ఫోను కవరేజి లేదు'
        
        elif distance_var == 'c':
            return 'మొబైల్ ఫోను కవరేజి గ్రామానికి 10 కి.మీ. కంటే ఎక్కువ దూరంలో లో ఉంది'
        elif distance_var == 'b':
            return 'మొబైల్ ఫోను కవరేజి గ్రామానికి 5-10 కి.మీ. సమీపం లో ఉంది'
        return 'మొబైల్ ఫోను కవరేజి గ్రామానికి 5 కి.మీ. కంటే తక్కువ సమీపంలో ఉన్నవి'
    
    return 'మొబైల్ ఫోను కవరేజి కూడా లభిస్తుంది'


# In[108]:


def get_internet_cafe_status(info_df, k):
    if type_check(info_df.loc[k, ('Internet Cafes / Common Service Centre (CSC) (Status A(1)/NA(2))')]) > 1:
        ...
        if pd.isna(distance_var):
            return 'ఇంటర్నెట్ లేదు'
        
        elif distance_var == 'c':
            return 'ఇంటర్నెట్ గ్రామానికి 10 కి.మీ. కంటే ఎక్కువ దూరంలో లో ఉంది'
        elif distance_var == 'b':
            return 'ఇంటర్నెట్ గ్రామానికి 5-10 కి.మీ. సమీపం లో ఉంది'
        return 'ఇంటర్నెట్ గ్రామానికి 5 కి.మీ. కంటే తక్కువ సమీపంలో ఉన్నవి'
    
    return 'ఇంటర్నెట్ కేఫ్లు కూడా లభిస్తుంది'


# In[88]:


def get_private_courier_status(info_df, k):
    if type_check(info_df.loc[k, ('Private Courier Facility (Status A(1)/NA(2))')]) > 1:
        ...
        if pd.isna(distance_var):
            return 'ప్రైవేటు కొరియర్ వంటి సౌకర్యాలు లేదు'
        
        elif distance_var == 'c':
            return 'ప్రైవేటు కొరియర్ వంటి సౌకర్యాలు గ్రామానికి 10 కి.మీ. కంటే ఎక్కువ దూరంలో లో ఉంది'
        elif distance_var == 'b':
            return 'ఇప్రైవేటు కొరియర్ వంటి సౌకర్యాలు గ్రామానికి 5-10 కి.మీ. సమీపం లో ఉంది'
        return 'ఇప్రైవేటు కొరియర్ వంటి సౌకర్యాలు గ్రామానికి 5 కి.మీ. కంటే తక్కువ సమీపంలో ఉన్నవి'
    
    return 'ప్రైవేటు కొరియర్ వంటి సౌకర్యాలు కూడా లభిస్తుంది'
